...
function format_s3_objects(
    content::AbstractVector{Pair{Tuple{String,String},D}}
) where {D<:AbstractDict}
    return format_s3_objects(OrderedDict(k => v for (k, v) in content))
end

function format_s3_objects(content::AbstractVector{Tuple{String,String}})
    return format_s3_objects(OrderedDict(k => Dict() for k in content))
end

function s3_patches!(content::AbstractDict, changes::Set=Set(Pair{Symbol,Any}[]))
    return [
        @patch function AWSS3.s3_list_objects(
            config::AWSConfig, bucket, path_prefix; kwargs...
        )
            results = []
        ...
        @patch function AWSS3.s3_get(config::AWSConfig, bucket, path; kwargs...)
            return codeunits(get(content[(bucket, path)], "Content", ""))
        end
        @patch function AWSS3.s3_exists(config::AWSConfig, bucket, path)
            return haskey(content, (bucket, path))
        end
        @patch function AWSTools.S3.s3_copy(
            config::AWSConfig, bucket, path; to_bucket=bucket, to_path=path, kwargs...
        )
            return push!(
                changes, :copy => Dict(:from => (bucket, path), :to => (to_bucket, to_path))
            )
        end
        @patch function AWSS3.s3_delete(config::AWSConfig, bucket, path; kwargs...)
            return push!(changes, :delete => (bucket, path))
        end
        @patch function AWSS3.s3_put(
            ...
            return push!(changes, :put => Dict(:dest => (bucket, path), :data => data))
        end
    ]
end
