...
# Install AWSTools.jl
ENV PKG_PATH $JULIA_PKGDIR/$JULIA_PKGVER/AWSTools
COPY . $PKG_PATH
WORKDIR $PKG_PATH

# Install AWSTools.jl prerequisite AWS CLI. Do not use `yum install aws-cli`
...
RUN yum -y install $PKGS && \
	...
	update-metadata && julia -e 'Pkg.update(); Pkg.resolve(); Pkg.build("AWSTools")' && \
